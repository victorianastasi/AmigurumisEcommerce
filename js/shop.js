...
function mostrarProdCompra(){
    ...
    for (let i = 0; i < carrito.length; i++) {
        document.getElementById(`subtotal${i}`).innerHTML = `<p>$${carrito[i].cantidad * carrito[i].precio}</p>`;
        ...
        $(`#plus${i}`).click(function(){
            ...
            if(carrito[i].cantidad < 10){
                ...
                document.getElementById(`inputCantidad${i}`).innerHTML = `<input class="input-cantidad" value="${carrito[i].cantidad}">`;
                document.getElementById(`subtotal${i}`).innerHTML = `<p>$${carrito[i].cantidad * carrito[i].precio}</p>`;
                localStorage.setItem("carrito", JSON.stringify(carrito));
                mostrarPrecioTotal();
               
                $(`#plus${i}`).css({color : "rgb(0, 0, 0)", backgroundColor: "rgb(212, 189, 184)", borderRadius: "15px"});
                setTimeout(function(){
        ...
        $(`#min${i}`).click(function(){
            ...
            if(carrito[i].cantidad > 1){
                ...
                $(`#min${i}`).css({color : "rgb(0, 0, 0)", backgroundColor: "rgb(212, 189, 184)", borderRadius: "15px"});
                setTimeout(function(){
...
function mostrarPrecioTotal(){
    ...
    document.getElementById("totalCompra").innerHTML = `<p class="total-compra-precio" id="totalCompraPrecio">Total: $${acuTotalPrecio}</p>`;
    document.getElementById("totalCompraPrecio").classList.add('animate__animated', 'animate__headShake');
}

function eliminarProductoCarrito(index) {
    document.getElementById(`row${index}`).classList.add('animate__animated', 'animate__fadeOutRight');
    setTimeout(function(){
